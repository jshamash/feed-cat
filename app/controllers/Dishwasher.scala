...
object Dishwasher extends Controller {
  ...
  def createDishwasher = Action {
    ...
    DB.withConnection { implicit c =>
      SQL("insert into Dishwashers(dishwasher_id, isClean) values ({id}, {isClean});").on('id -> uuid, 'isClean -> false).executeInsert()
    }
    Ok(uuid)
  }
  ...
  def getDishwasher(id: String) = Action {
    ...
    val isClean = DB.withConnection { implicit c =>
      val selectDishwasher = SQL("Select isClean from Dishwashers where dishwasher_id={id}").on('id -> uuid.toString)
  ...
  def toggleDishwasher(id: String) = Action {
    ...
    val isClean = DB.withConnection { implicit c =>
      ...
      // update dishwasher
      SQL("update Dishwashers set isClean={isClean} where dishwasher_id=({id});").on('id -> uuid, 'isClean -> !isClean).executeUpdate()
